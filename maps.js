...
polygonTemplate.tooltipText = "{name}: {customValue}";
...
// Function to get content based on region ID
function getStateContent(regionId) {
    // You can customize this function to load content from different HTML files
    switch (regionId) {
      case "IN-MH":
...
// Add click event listener to dynamically change content
polygonTemplate.events.on("hit", function (ev) {
    console.log('clicked');
    // Get the ID of the clicked region
    let regionId = ev.target.dataItem.dataContext.id;
    console.log(regionId);
    // Get the content file path based on the region ID
    let contentFilePath = getStateContent(regionId);
    console.log(contentFilePath);
    
    // Fetch the content from an external HTML file
    $.get(contentFilePath, function () {
        // Open the content in a new window or tab
        let newWindow = window.open(contentFilePath);
        
        // Set the HTML content after the new window is fully loaded
        newWindow.onload = function () {
            // Update specific elements in the new window
            let newWindowDocument = newWindow.document;
            let title = newWindowDocument.getElementById("stateTitle");
            if (title) {
                title.innerHTML = ev.target.dataItem.dataContext.name;
            }
            let UNESCO = newWindowDocument.getElementById("stateContent");
            if (UNESCO) {
                UNESCO.innerHTML = ev.target.dataItem.dataContext.customValue;
            }
        };
    });
});
